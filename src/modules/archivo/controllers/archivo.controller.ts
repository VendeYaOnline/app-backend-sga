...
import { ApiTags, ApiBearerAuth, ApiConsumes, ApiOperation, ApiResponse, ApiParam, ApiBody } from '@nestjs/swagger';
...
export class ArchivoController {
  ...
  @ApiOperation({ summary: 'Subir archivo', description: 'Sube un archivo y lo asocia a una entidad del sistema' })
  ...
  @ApiBody({ description: 'Archivo a subir con metadatos', schema: { type: 'object', properties: { file: { type: 'string', format: 'binary' }, entidad: { type: 'string' }, entidadId: { type: 'string' }, proposito: { type: 'string' } } } })
  async upload(
  ...
  @Get(':uuid')
  @ApiOperation({ summary: 'Descargar archivo por UUID', description: 'Descarga un archivo desde el sistema usando su UUID' })
  ...
  @ApiOperation({ summary: 'Listar archivos por entidad', description: 'Retorna los archivos asociados a una entidad específica' })
  @ApiResponse({ status: 200, description: 'Lista de referencias de archivos' })
  @ApiParam({ name: 'entidad', type: String, description: 'Tipo de entidad (ej: SOLICITUD, EVENTO)' })
  @ApiParam({ name: 'entidadId', type: Number, description: 'ID de la entidad' })
  async findByEntidad(
  ...
  @ApiOperation({ summary: 'Eliminar referencia de archivo (soft delete)', description: 'Elimina lógicamente una referencia de archivo' })
  @ApiResponse({ status: 200, description: 'Referencia eliminada exitosamente' })
